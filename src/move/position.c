...
*/
static void	get_position(t_stack **s)
{
	t_stack	*tmp;
	int		i;
...
*/
static int	get_target(t_stack **a, int b_idx, int tar_idx, int tar_pos)
{
	t_stack	*tmp_a;
	...
	while (tmp_a)
	{
		if (tmp_a->index > b_idx && tmp_a->index < tar_idx)
		{
			tar_idx = tmp_a->index;
			tar_pos = tmp_a->pos;
		}
